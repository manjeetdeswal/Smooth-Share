...
import com.thenotesgiver.smooth_share.databinding.LayoutSplashBinding
import com.thenotesgiver.smooth_share.databinding.ListPermissionBinding
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.parcelize.IgnoredOnParcel
import kotlinx.parcelize.Parcelize

import com.thenotesgiver.smooth_share.util.Permissions
import com.thenotesgiver.smooth_share.util.Permissions.Permission
import com.thenotesgiver.smooth_share.util.Resources.attrToRes
import com.thenotesgiver.smooth_share.util.Resources.resToColor
...
class WelcomeActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= 23) adapter.add(
            IntroductionFragmentStateAdapter.PageItem(
                getString(R.string.introduction_set_up_permissions),
                IntroductionPermissionFragment::class.java.name
            )
        )

        adapter.add(
            IntroductionFragmentStateAdapter.PageItem(
                getString(R.string.introduction_finished),
                IntroductionAllSetFragment::class.java.name
            )
        )
        ...
        nextButton.setOnClickListener {
           ...
           if (viewPager.currentItem + 1 < adapter.itemCount) {
                ...
                if (Permissions.checkRunningConditions(applicationContext)) {
                    ...
                    startActivity(Intent(this@WelcomeActivity, SharingActivity::class.java))
