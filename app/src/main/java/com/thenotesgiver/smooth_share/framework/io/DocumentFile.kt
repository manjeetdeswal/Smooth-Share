...
package com.thenotesgiver.smooth_share.framework.io
...
class DocumentFile private constructor(
    ...
    private val archive: ArchiveData?,
) : Parcelable {
    val parent: DocumentFile?
        get() = data?.parent ?: file?.parentFile?.let { DocumentFile(it) } ?: archive?.parent

    val filePath: String?
        get() = file?.canonicalPath

    private constructor(originalUri: Uri, data: Data) : this(originalUri, data = data, file = null, archive = null)

    private constructor(file: File) : this(Uri.fromFile(file), file = file, data = null, archive = null)

    private constructor(originalUri: Uri,archive: ArchiveData) : this(originalUri, archive = archive, file = null, data = null)

    fun canRead(): Boolean = file?.canRead() ?: data?.canRead() ?: archive?.canRead() ?: false

    fun canWrite(): Boolean = file?.canWrite() ?: data?.canWrite() ?: archive?.canWrite() ?: false
    ...
    fun exists(): Boolean = data?.exists ?: file?.exists() ?: archive?.exists ?: false
    ...
    fun isDirectory(): Boolean = data?.isDirectory() ?: file?.isDirectory ?: archive?.isDirectory() ?: throw IllegalStateException()

    fun isFile(): Boolean = data?.isFile() ?: file?.isFile ?: archive?.isFile() ?: throw IllegalStateException()

    fun isTreeDocument(): Boolean = data != null

    fun isVirtual(): Boolean = data?.isVirtual() ?: false

    fun getLastModified(): Long = data?.lastModified ?: file?.lastModified() ?: archive?.lastModified ?: throw IllegalStateException()

    fun getLength(): Long = data?.length ?: file?.length() ?: archive?.length ?: throw IllegalStateException()

    fun getName(): String = data?.name ?: file?.name ?: archive?.name ?: throw IllegalStateException()

    fun getSecureUri(context: Context, authority: String): Uri {
        if (SDK_INT < Build.VERSION_CODES.M || data != null || archive != null) {
            return getUri()
        } else if (file != null) {
    ...
    fun fromArchived(): Boolean = archive != null

    fun getUri(): Uri = data?.uri ?: archive?.uri ?: originalUri

    fun getType(): String = data?.type ?: file?.let {
        if (isDirectory() && SDK_INT >= 19) MIME_TYPE_DIR else getMimeType(getName())
    } ?: archive?.let {
        if (isDirectory() && SDK_INT >= 19) MIME_TYPE_DIR else getMimeType(getName())
    }
    ?: throw IllegalStateException()
    ...
    data class ArchiveData(
        ...
        fun isDirectory(): Boolean = dir

        fun isFile(): Boolean = !isDirectory()

        companion object {
            fun from(parent: DocumentFile?, uri: Uri, name: String, dir: Boolean, size: Long, lastModified: Long): ArchiveData = ArchiveData(
                parent,
                uri,
                name,
                dir,
